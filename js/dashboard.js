...
const Dashboard = (() => {
  ...
  function render() {
    ...
    const variances = phaseData
      .filter(ph => budget > 0 && ph.total > (budget / 8))
      .map(ph => ({ ...ph, over: ph.total - (budget / 8) }));

    return `
    <div class="phase-workspace active">
      <div class="phase-header">
        <div class="phase-title-block">
          <div class="phase-title">📊 Project Dashboard</div>
      ...
      <div class="dashboard-grid">
        ...
        <div class="stat-card">
          ...
          <div class="stat-card-sub">${budget > 0 ? (remaining >= 0 ? 'Available to spend' : '⚠ Budget exceeded') : 'Set a budget to track'}</div>
        </div>
      </div>
          ...
          <div class="donut-container">
            ...
            <div class="donut-legend">
              ...
              <div class="donut-legend-item" style="margin-top:8px;padding-top:8px;border-top:1px solid var(--charcoal-border)">
                ...
                <span>Avg completion: ${Math.round(proj.phases.reduce((s,p) => s + p.completion, 0) / 8)}%</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="dashboard-chart-card" style="border-color:rgba(239,68,68,0.15)">
        <div class="dashboard-chart-title" style="color:var(--warning)">⚠ Variance Alerts</div>
        ${variances.map(v => `
  ...
  function renderDonut(pct, spent, budget) {
    ...
    const color = pct > 100 ? 'var(--warning)' : pct > 85 ? '#D97706' : 'var(--amber)';
    ...
    <div class="dashboard-chart-card">
      ...
      <div class="dashboard-grid" style="margin-bottom:0">
        ...
        <div class="stat-card" style="padding:12px 14px">
          ...
          <div class="stat-card-value" style="font-size:18px;color:${totalOwed > 0 ? '#D97706' : 'var(--success)'}">${F.fmtFull(totalOwed)}</div>
        </div>
      </div>
    </div>`;
  }
